    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background-color: #ffffff; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
        <tr>
            <td style="background-color: #4a90e2; text-align: center;">
                <p style="font-size: 54px; line-height: 54px; font-weight: 800; color: #ffffff; margin: 20px 0;">SubDub</p>
            </td>
        </tr>
        <tr>
...
export const emailTemplates = [
  {
    ...
    daysLeft: 7, // Added this helper property for easier matching logic
    generateSubject: (data: EmailData) => `📅 Reminder: Your ${data.subscriptionName} Subscription Renews in 7 Days!`,
    generateBody: (data: EmailData) => generateEmailTemplate({ ...data, daysLeft: 7 }),
  },
  {
    label: "5 days before reminder",
    daysLeft: 5,
    generateSubject: (data: EmailData) => `⏳ ${data.subscriptionName} Renews in 5 Days – Stay Subscribed!`,
    generateBody: (data: EmailData) => generateEmailTemplate({ ...data, daysLeft: 5 }),
  },
  {
    label: "2 days before reminder",
    daysLeft: 2,
    generateSubject: (data: EmailData) => `🚀 2 Days Left!  ${data.subscriptionName} Subscription Renewal`,
    generateBody: (data: EmailData) => generateEmailTemplate({ ...data, daysLeft: 2 }),
  },
  {
    label: "1 days before reminder",
    daysLeft: 1,
    generateSubject: (data: EmailData) => `⚡ Final Reminder: ${data.subscriptionName} Renews Tomorrow!`,
    generateBody: (data: EmailData) => generateEmailTemplate({ ...data, daysLeft: 1 }),
  },
];